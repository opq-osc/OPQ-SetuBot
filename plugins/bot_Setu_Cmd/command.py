...
from botoy import S, jconfig, GroupMsg
...
class CMD:
    ...
    def change_dict(self, dicta: dict, lista: list, change, ret=''):
        ...
        if len(lista) == 1:
            ...
            return '{}: {}-{}\n↓↓↓↓\n{}: {}-{}'.format(ret, rt_befeore[lista[0]], type(rt_befeore[lista[0]]), ret
                                                       , dicta[lista[0]], type(dicta[lista[0]]))
        lista.pop(0)
    ...
    def main(self):
        if auth := self.auth():
            if auth == 1:
                if res := re.match(r'_cmd [G,g] (\d+) (.*) (.*):(.*)', self.ctx.Content):  # 万能修改
