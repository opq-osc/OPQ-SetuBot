...
def setuapi_0(tag='', num=1, r18=False):
    ...
    if status_code == 200:
        ...
        for i in setu_data['data']:
            msg.append(pixiv_url(i['title'], i['artwork'], i['author'], i['artist'], i['page']))
...
def setuapi_1(keyword='', num=1, r18=False):
    ...
    if status_code == 200 and setu_data['code'] == 0:
        print('从api获取到{0}条数据'.format(len(setu_data['data'])))  # 打印获取到多少条
        for i in setu_data['data']:
            msg.append(pixiv_url(i['title'], i['pid'], i['author'], i['uid'], i['p']))
            url.append(i['url'])
    return setu_data['code'], url, msg


def pixiv_url(title, artworkid, author, artistid, page):  # 拼凑消息
    purl = "www.pixiv.net/artworks/" + str(artworkid)  # 拼凑p站链接
    uurl = "www.pixiv.net/users/" + str(artistid)  # 画师的p站链接
    if type(page) == int:
        page = 'p' + str(page)
    msg = '标题:{title}\r\n图片链接:{purl}\r\npage:{page}\r\n作者:{author}\r\n作者链接:{uurl}'.format(title=title, purl=purl, page=page, author=author,
                                                                                         uurl=uurl)
    return msg
